...
public class Application implements CoreController {
  ...
  private final Deque<AppModule>                repeat, loop, once;
  ...
  public Application() {
    ...
    this.once = new LinkedBlockingDeque<>();
  ...
  public void addModule(final AppModule m) {
    ...
    if (m != null && m.getRunMode() != null) {
      switch (m.getRunMode()) {
        case LOOP:
          if (!this.loop.contains(m)) {
            this.loop.offerLast(m);
            if (!m.isRunning())
  ...
  private void setup() {
    ...
    Runtime.getRuntime().addShutdownHook(new Thread(() -> {
      ...
      this.once.forEach(AppModule::exit);
    }));
